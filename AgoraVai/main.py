...
def move_straight():
    while infra_red_in1.proximity > 14:
        tank_drive.on(20, 20)
        odometry.get_pos()  # Atualiza a posição do robô
    tank_drive.off(brake=True)

def turn_left(fist_read):
    gyro_sensor.reset()
    while ultrasonic_sensor_in3.distance_centimeters - 2 > fist_read:
        print(gyro_sensor.angle)
        tank_drive.on(-5, 5)
        odometry.get_pos() 
    tank_drive.off(brake=True)

def turn_right(fist_read):
    gyro_sensor.reset()
    while ultrasonic_sensor_in4.distance_centimeters - 2 > fist_read:
        print(gyro_sensor.angle)
        tank_drive.on(5, -5)
        odometry.get_pos()  
    tank_drive.off(brake=True)
...
while True:
    ...
    if ultrasonic_sensor_in3.distance_centimeters > ultrasonic_sensor_in4.distance_centimeters:
        turn_left(infra_red_in1.proximity)
    else:
        turn_right(infra_red_in1.proximity)
